...
LuaCEmbedTable  *LuaCEmbedTable_get_sub_table(LuaCEmbedTable *self, const char *name){
    ...
    sprintf(full_sub_table_name,"%s_%s",self->global_buffer,name);
    ...
    LuaCEmbedTable  *possible = privateLuaCEmbedTableArray_find_by_name(
            (privateLuaCEmb //checking if exist a global object
    lua_getglobal(self->main_object->state,full_sub_table_name);edTableArray*)self->sub_tables,
            name
            );
    if(possible){
        return possible;
